...
import { hasOwnProperty } from './mixed'
...
export function getMatchedRoutes(route) {
  if (
    route === null ||
    (typeof route !== 'string' && typeof route !== 'object')
  ) {
    return []
  }
  let { matched } = route
...
export function getDefaultRouteComponent(route) {
  ...
  if (to !== null && typeof to === 'object' && hasOwnProperty(to, 'default')) {
    return to['default']
  }
  return null
}
...
export function getMatchedRoutesByView(view, root, special) {
  ...
  const parentRouteIndex = matched.findIndex((route) =>
    isSameRoute(route, rootPath)
  )
