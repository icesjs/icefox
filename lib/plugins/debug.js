...
function printMessage(name, level, ...args) {
  ...
  const namespace = `${prefix}:${level}:${(name = isManager
    ? args.shift()
    : name)}`
...
function enableDebug() {
  const namespaces =
    preservedNamespaces || getCurrentDebugNamespaces().join(',')
  preservedNamespaces = ''
...
export default {
  ...
  install(Vue, config) {
    ...
    const stub =
      config && typeof config.propertyName === 'string'
        ? config.propertyName
        : '$debug'
