...
function paramsFilter(path, params, baseParams) {
  ...
  return path
    .replace(/(.?){\s*(.*?)\s*(\\|)}/g, ($0, $1, $2, $3) => {
      ...
      let query
      if (hasOwnProperty(params, $2)) {
        query = params[$2]
        // 从查询参数配置中清除已替换的参数项
        delete params[$2]
      } else if (hasOwnProperty(baseParams, $2)) {
...
function urlFilter(config, baseParams) {
  ...
  // 所以这里要对这些格式进行分割解析
  const splitUrlReg = new RegExp(
    `^\\s*(?:(${methods.join('|')})\\s+)?(.+)\\s*$`,
    'i'
  )
  const [, method, path] =
    splitUrlReg.exec(typeof url === 'string' ? url : '') || []
...
function makeCancelablePromise(promise, cancel) {
  ...
  for (const method of ['cancel', 'then', 'catch', 'finally']) {
    if (!hasOwnProperty(promise, method)) {
          ...
          method === 'cancel'
            ...
            : (...args) =>
                makeCancelablePromise(
                  proto[method].apply(promise, args),
                  cancel
                ),
      })
    }
  ...
  return function accessor(config) {
    ...
    const resolve = async (prop) => {
      ...
      const value = (response ||
        error.response || { statusText: '', message: '' })[prop]
      // 正常响应
      if (!error) {
        return value
      }
...
function createHttpRequest(defaults) {
  ...
  const mockPlugin = buildInPlugins.find(
    (plugin) => plugin && plugin.name === 'mock'
  )
  if (mockPlugin) {
...
const plugin = {
  ...
  install(Vue, defaults, plugins) {
    ...
    const stub =
      defaults && typeof defaults.propertyName === 'string'
